...
export default function RideCreate() {
  ...
  // Auto-enable RSVP for overnight/multi-day trips
  const isOvernightTrip = rideType === 'overnight' || rideType === 'multi_day';

  const createMutation = useMutation({
    mutationFn: (data: RideFormData) => ridesApi.create({
      ...data,
      estimatedDistance: data.estimatedDistance ? Number(data.estimatedDistance) : undefined,
      estimatedDuration: data.estimatedDuration ? Number(data.estimatedDuration) * 60 : undefined, // Convert hours to minutes
    ...
    onError: (error: { response?: { data?: { error?: string | Array<{ message: string }> } } }) => {
      ...
      if (Array.isArray(err)) {
        ...
        toast.error(err || 'Failed to create ride');
      }
    },
  });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-display font-bold">Create New Ride</h1>
